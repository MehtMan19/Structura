...
function loadPrecedents(): PrecedentFile[] {
  try {
    return JSON.parse(localStorage.getItem("structura_precedents") ?? "[]");
  } catch {
    return [];
  }
}

function savePrecedents(files: PrecedentFile[]) {
  localStorage.setItem("structura_precedents", JSON.stringify(files));
}
...
export function Precedents() {
  const [precedents, setPrecedents] = useState<PrecedentFile[]>(loadPrecedents);
  ...
  const addFiles = useCallback(
    (files: FileList | File[], category: PrecedentCategory) => {
      const valid = Array.from(files).filter(
        (f) => ACCEPTED_TYPES.includes(f.type) || ACCEPTED_EXTENSIONS.test(f.name)
      );
      if (valid.length === 0) return;

      const newPrecedents: PrecedentFile[] = valid.map((f) => ({
      ...
      const updated = [...newPrecedents, ...precedents];
      setPrecedents(updated);
      savePrecedents(updated);
    },
    [precedents]
  );
              ...
              <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
                {catFiles.map((file) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => removePrecedent(file.id)}
